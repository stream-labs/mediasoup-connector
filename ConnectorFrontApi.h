...
struct ConnectorFrontApiHelper {
	static void
	createInterfaceObject(const std::string &routerRtpCapabilities_Raw,
			      calldata_t *cd);
	static bool createVideoProducerTrack(calldata_t *cd,
					     const std::string &input);
	static bool createAudioProducerTrack(calldata_t *cd,
					     const std::string &input);
	static bool createProducerTrack(const std::string &kind, calldata_t *cd,
					const std::string &input);
	static bool
	createConsumer(MediaSoupInterface::ObsSourceInfo &obsSourceInfo,
		       const std::string &params, const std::string &kind,
		       calldata_t *cd);
	...
	static bool onConnect(const std::string &clientId,
			      const std::string &transportId,
			      const json &dtlsParameters);
	static bool onProduce(const std::string &clientId,
			      const std::string &transportId,
